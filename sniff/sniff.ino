...
void loop() {
  ...
  if (changetime > 0 && changetime < millis() - 100)
  {
    Serial.println();
    for (int i = 0; i < NUM_CHARS; i++)
    {
      Serial.print(LCD_shadow[i]);
      if ((i + 1) % (NUM_CHARS / 4) == 0)
      {
        Serial.println();
      }
    }
...
void decode() {
  if (rw == 0)
  {
    ...
    if (rs == 1)
    {
      ...
      charIndex = (charIndex + 1) % NUM_CHARS; //changed here era 32
    }
    else
    {
      // command received
...
void evalCommand(uint8_t command)
{
  ...
  if ((command & LCD_SETDDRAMADDR_MASK) == LCD_SETDDRAMADDR)
  {
    ...
    else if ( (command & 0x7f) >= 64 && (command & 0x7f) < 80 )
    {
      charIndex = (command & 0x7f) - 48;
    }
    else if ( (command & 0x7f) >= 16 && (command & 0x7f) < 32 )
    {
      charIndex = (command & 0x7f) + 16;
    }
    else
    {
      charIndex = (command & 0x7f) - 32;
    }
    return;
  }
